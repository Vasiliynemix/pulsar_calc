...
class Lexicon:
    ...
    LEXICON = {
        "msg_send": {
            # Редактирование самой первой команды start(переменная on_start_register) через nano
            # - удалить прошлое сообщение(кнопкой delete, выделение в терминало не работает)
            # - потом скопированный текст вставить между ковычками(обязательно должно быть 3 ковычки по бокам)
            # - вставка в этом терминале через ctrl+shift+W
            # - после изменений нажать ctrl+x, потом y для подтверждения изменения
            ...
            "cmd_rate_set_err": (
                "⚠️Чтобы установить актуальный курс введи:\n"
                "/rate_ask значение курса\n"
                "/rate_bid значение курса\n\n"
                "💡 Например:\n"
                "/rate_ask 95.5 или /rate_bid 94.5\n\n"
                "/rate_ask - курс продажи\n"
                "/rate_bid - курс покупки\n\n"
                "Вводить нужно актуальные значения без добавления +1, -1\n\n"
                "Значение должно быть целым или вещественным числом: 95, 95,5, 95.5"
            ),
            "cmd_rate_set_ok": "курс {} установлен\n1 usdt = {} ₽",
            ...
            "algorithm_calculate": (
                ...
                "<b>Доход:</b> {profit_black_day} ₽ ({profit_black_day_btc}) BTC.\n"
                "<b>Расход:</b> {cost_day} ₽ ({cost_day_btc}) BTC.\n"
                "<b>Прибыль:</b> {profit_white_day} ₽ ({profit_white_day_btc}) BTC.\n\n"
                "<b>Расчет прибыли за месяц:</b>\n"
                "<b>Доход:</b> {profit_black_moth} ₽ ({profit_black_moth_btc}) BTC.\n"
                "<b>Расход:</b> {cost_moth} ₽ ({cost_moth_btc}) BTC.\n"
                "<b>Прибыль:</b> {profit_white_moth} ₽ ({profit_white_moth_btc}) BTC.\n"
                "<b>----------------------------------</b>\n\n"
...
class LexiconMsgSend:
    def __init__(self, lexicon: dict[str, str | dict[str, str]]) -> None:
        ...
        self.cmd_rate_set_err = self._lexicon["cmd_rate_set_err"]
        self.cmd_rate_set_ok = self._lexicon["cmd_rate_set_ok"]
...
class LexiconMsgKbName:
    def __init__(self, lexicon: dict) -> None:
        ...
        self.chat_to_manager = LexiconMsgKbNameReplyInline(
            self._lexicon["chat_to_manager"]
        )
